...
export const VoiceButton = ({ id, style, text, audio }) => {
	...
	console.log('🚀 ~ file: VoiceButton.js ~ line 8 ~ VoiceButton ~ isAudioPlaying', isAudioPlaying);

	const handleVoiceButtonClick = useCallback(id => {
		console.log({ id });
		setIsAudioPlaying(isAudioPlaying => ({ ...isAudioPlaying, [id]: true }));
	...
	const handleAudioEnded = useCallback(id => {
		...
		setIsAudioPlaying(isAudioPlaying => ({ ...isAudioPlaying, [id]: false }));
	}, []);

	return (
		<button key={id} className={styles.voice} onClick={() => handleVoiceButtonClick(id)}>
			<div style={style}>
				...
				<span className={classNames('progress', { active: isAudioPlaying[id] })} style={{ animationDuration: '10s' }} />
			</div>
